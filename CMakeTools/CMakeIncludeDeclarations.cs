...
namespace CMakeTools
{
    ...
    class CMakeIncludeDeclarations : Declarations
    {
        ...
        // Flag indicating whether the list should show packages instead of
        // modules.
        private bool _showPackages;

        public CMakeIncludeDeclarations(string sourceFilePath, bool showPackages = false)
        {
            ...
            if (showPackages)
            {
                files = Directory.EnumerateFiles(dirPath, "Find*.cmake");
                files = files.Select(Path.GetFileNameWithoutExtension);
                files = files.Select(x => x.Substring(4));
            }
            else
            {
                files = Directory.EnumerateFiles(dirPath, "*.cmake");
        ...
        {
            // If we're showing packages, return the index for a library.
            if (_showPackages)
            {
                return 193;
            }
